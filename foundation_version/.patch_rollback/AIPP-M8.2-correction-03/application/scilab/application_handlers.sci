function response=p55HandleReferencePing(message)
 response=struct("protocol",P2_PROTOCOL_NAME,"protocol_version",P2_PROTOCOL_VERSION,"type","application.example.ping.response","request_id",message.request_id,"status","success","payload",struct("message","pong","source","application-owned-handler","application_version","0.7.0-m7"));
endfunction
function response=p56HandleReferenceSum(message)
 ...
 response=struct("protocol",P2_PROTOCOL_NAME,"protocol_version",P2_PROTOCOL_VERSION,"type","application.example.sum.response","request_id",message.request_id,"status","success","payload",struct("sum",sum(values),"count",size(values,"*"),"source","application-owned-handler","application_version","0.7.0-m7"));
endfunction
function response=pAippHandleStatus(message)
 global P55_APPLICATION_STARTED P55_AIPP_MODEL_LOADED;
 response=struct("protocol",P2_PROTOCOL_NAME,"protocol_version",P2_PROTOCOL_VERSION,"type","application.aipp.status.response","request_id",message.request_id,"status","success","payload",struct("application_id","application.aipp","application_name","AIPP Inflator Model Viewer","application_version","0.7.0-m7","ready",P55_APPLICATION_STARTED,"model_loaded",P55_AIPP_MODEL_LOADED,"foundation_release","AI-1.0.0","runtime_foundation","P6.4.0-0.1","minimum_scilab_version","2026.1.0"));
endfunction
function response=pAippHandlePyrolist(message)
    ...
    payload=struct("source","application/data/pyrolist.json","count",size(names,"*"),"names",names,"application_version","0.7.0-m7");
...
function response=pAippHandleMaterials(message)
    ...
    payload=struct("source","application/data/material_list.json","count",size(names,"*"),"names",names,"materials",materials,"application_version","0.7.0-m7");
...
function response=pAippHandleDeckSave(message)
    ...
    if saveAs|target=="" then target=uiputfile("*.json",".",suggested,"Save AIPP input deck");end
    if target=="" then
