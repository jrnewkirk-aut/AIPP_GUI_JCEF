...
# Tree JSON View and Edit Controls

Within the GUI space for editing AIPP input JSON files there must be a tab for a tree viewer of the JSON. This should show the hierarchy and be better looking and more intuitive than the RAW JSON

The tab must be called "Tree"

The JSON tree editor must have the following charactistics:

1. Scrollable
2. Editable
3. Collapsible and Expandable
   1. individual chunks of the JSON should be expandable and collapsible.
   2. There should be a button within the window for the JSON tree viewer itself to expand and collapse all.
      1. The button should not be outside the window for the tree viewer itself to avoid confusion with the other viewing and editing methods.
4. Numbering
...
5. An apply button and an undo button must be present within the window for the JSON tree viewer itself.
6. A search bar should be available in the json tree viewer itself
   1. This should reduce the visible JSON to only the lines that contain the field in the search bar.

# JSON State Ownership Model


Primary source of truth:
- JSON object in browser memory
